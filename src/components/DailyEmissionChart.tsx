...
const DailyEmissionChart = ({ className = "h-[22rem]" }: Props) => {
  return (
    <ChartContainer config={chartConfig} className={`w-full ${className}`}>
      ...
      >
        ...
        <Line
          ...
          strokeWidth={2}
          dot={{ r: 3, fill: chartConfig.CO.color }}
          activeDot={{ r: 5 }}
        />
        <Line
          type="monotone"
          dataKey="NO_2"
          stroke={chartConfig.NO_2.color}
          strokeWidth={2}
          dot={{ r: 3, fill: chartConfig.NO_2.color }}
          activeDot={{ r: 5 }}
        />
        <Line
          type="monotone"
          dataKey="PM2_5"
          stroke={chartConfig.PM2_5.color}
          strokeWidth={2}
          dot={{ r: 3, fill: chartConfig.PM2_5.color }}
          activeDot={{ r: 5 }}
        />
        <Line
          type="monotone"
          dataKey="SOx"
          stroke={chartConfig.SOx.color}
          strokeWidth={2}
          dot={{ r: 3, fill: chartConfig.SOx.color }}
          activeDot={{ r: 5 }}
        />
      </LineChart>
    </ChartContainer>
